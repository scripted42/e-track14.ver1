    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h1 class="h3 mb-1 text-gray-800">✏️ Edit Pengguna</h1>
                    <p class="text-muted mb-0">Perbarui informasi pengguna sistem</p>
                </div>
                <div class="d-flex gap-2">
                    <a href="{{ route('admin.users.show', $user) }}" class="btn btn-outline-info btn-modern">
                ...
                <div class="card-header bg-primary text-white">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <small class="text-white opacity-75">{{ $user->email }} • {{ ucfirst($user->role->role_name ?? 'N/A') }}</small>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="row">
                                    @foreach($roles as $role)
                                        <div class="col-md-4 mb-3">
                                            <label class="role-card {{ old('role_id', $user->role_id) == $role->id ? 'selected' : '' }}" for="role_{{ $role->id }}">
                                                <input type="radio" 
                                                       id="role_{{ $role->id }}" 
                                                       name="role_id" 
                                                       value="{{ $role->id }}"
                            ...
                            <div class="col-12">
                                <div class="alert alert-info border-0" style="background: linear-gradient(135deg, #e0f2fe 0%, #b3e5fc 100%);">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            <h6 class="mb-1 text-info">Informasi Akun</h6>
                                            <p class="mb-0 text-info">
                                                <strong>ID Pengguna:</strong> #{{ str_pad($user->id, 4, '0', STR_PAD_LEFT) }} | 
                                                <strong>Dibuat:</strong> {{ \Carbon\Carbon::parse($user->created_at)->format('d F Y') }} | 
                                                <strong>Terakhir Update:</strong> {{ \Carbon\Carbon::parse($user->updated_at)->diffForHumans() }}
                                            </p>
                                        </div>
                                    </div>
                                </div>
